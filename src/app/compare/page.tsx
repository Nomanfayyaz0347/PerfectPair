    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 py-4 px-4">
      <div className="max-w-md mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-lg p-4 mb-4">
          <div className="mb-4">
            <h1 className="text-lg font-bold text-gray-800 mb-2">Compare Profiles</h1>
            <p className="text-xs text-gray-600">Enter two names to check their compatibility</p>
          </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <select
                  value={profile1}
                  onChange={(e) => setProfile1(e.target.value)}
                  className="w-full px-3 py-2 text-sm border border-gray-300 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 bg-white"
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-emerald-500 active:bg-emerald-600 disabled:bg-gray-400 text-white py-3 rounded-lg text-sm font-medium transition-all active:scale-95"
            >
              ...
              {result.match ? (
                <>
                  <div className="flex items-center justify-between mb-3">
                    <h2 className="text-base font-semibold text-gray-800">Match Results</h2>
                    <span className="text-lg font-bold text-emerald-600">{result.matchPercentage}% Match</span>
                  </div>
                ...
                <div className="text-center py-3">
                  <p className="text-base font-medium text-red-600">Not Compatible</p>
                  {result.reason && <p className="mt-2 text-sm text-gray-600">{result.reason}</p>}
                </div>
              )}
            </div>
          )}
        ...
        <div className="text-center">
          ...
          <div className="space-y-4">
            ...
            {/* Details Popup */}
            {detailsPopup && (
              <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" onClick={() => setDetailsPopup(null)}>
                <div className="bg-white rounded-xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between mb-3 pb-3 border-b">
                      <div>
                        <h2 className="text-base font-bold text-gray-800">Match Details</h2>
                        <p className="text-sm text-gray-600">{detailsPopup.name}</p>
                      </div>
                      <div className="text-right">
                        <div className="flex gap-2 items-baseline">
                          <div>
                            <div className="text-xl font-bold text-purple-600">{detailsPopup.requirementMatchPercentage || detailsPopup.percentage}%</div>
                            <div className="text-xs text-gray-500">Requirements</div>
                          </div>
                          <div className="text-gray-400">|</div>
                          <div>
                            <div className="text-xl font-bold text-emerald-600">{detailsPopup.directMatchPercentage || 0}%</div>
                      ...
                      <div className="mb-4">
                        ...
                        <div className="grid gap-2">
                          {detailsPopup.requirementMatchingFields.map((field, idx) => (
                            <div key={idx} className="flex items-center p-2 bg-purple-50 border border-purple-200 rounded-lg">
                              ...
                              <span className="text-sm font-medium text-purple-800 capitalize">{field.replace(/([A-Z])/g, ' $1').trim()}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {detailsPopup.directMatchingFields && detailsPopup.directMatchingFields.length > 0 && (
                      <div className="mb-4">
                        <h3 className="text-base font-semibold text-emerald-600 mb-2 flex items-center">
                          <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20">
                            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"/>
                          </svg>
                        ...
                        <div className="grid gap-2">
                          {detailsPopup.directMatchingFields.map((field, idx) => (
                            <div key={idx} className="flex items-center p-2 bg-emerald-50 border border-emerald-200 rounded-lg">
                              <svg className="w-5 h-5 text-emerald-600 mr-2 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd"/>
                              </svg>
                              <span className="text-sm font-medium text-emerald-800 capitalize">{field.replace(/([A-Z])/g, ' $1').trim()}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="mb-3">
                        ...
                        <div className="grid gap-2">
                          {Object.keys(detailsPopup.profiles.profile1).map((field) => {
                            ...
                            return (
                              <div key={field} className="p-2 bg-red-50 border border-red-200 rounded-lg">
                    ...
                    <button
                      ...
                      className="w-full mt-3 bg-gray-500 text-white py-3 px-4 rounded-lg active:bg-gray-600 text-sm font-medium transition-all active:scale-95"
                    >
                      Close
                    </button>
                  </div>
